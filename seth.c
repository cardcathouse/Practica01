...
int main(int argc, char *argv[])
{
  ...
  eh->ether_type = htons(70 + strlen (Cadena));  /*Recordemos, va al protocolo o la longitud del paquete*/
  tx_len += sizeof(struct ether_header);
  strcpy (sendbuf+tx_len, Cadena);
  tx_len = tx_len + strlen (Cadena);   
  for (i=0; i<70; i++) sendbuf[tx_len++] = 0xBB;  /*Se llena con 70 caracteres mas*/
  ...
  time_t seg;
  printf("Inserta el tiempo en segundos: ");
  scanf("%ld", &seg);
  time_t startTime =  time(NULL);
  int packCounter = 0;

  /*Envio del paquete*/
  while(time(NULL) - startTime < seg)
  {
